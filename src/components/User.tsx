import React from 'react';
import { Button, Form, Grid, Header, Image, Segment } from 'semantic-ui-react'
import Profile from './Profile';
import Navbar from './Navbar'

const userPwdDB: {[index: string]: string} = {
    mike: "1234",
    lucy: "2345",
    tom: "3456"
}

const userGenderDB: {[index: string]: string} = {
    mike: "Male",
    lucy: "Female",
    tom: "Male"
}

const userDobDB: {[index: string]: Date} = {
    mike: new Date("1991-10-26"),
    lucy: new Date("1999-10-01"),
    tom: new Date("1983-03-21")
}

const userPlaceDb: {[index: string]: string} = {
    mike: "Edmonton",
    lucy: "Calgary",
    tom: "Red Deer"
}

const userPhotoDb: {[index: string]: string} = {
    mike: "https://react.semantic-ui.com/images/avatar/large/matthew.png",
    lucy: "https://react.semantic-ui.com/images/avatar/large/rachel.png",
    tom: "https://react.semantic-ui.com/images/avatar/large/elliot.jpg"
}

interface IUserProps {

}

interface IUserState {
    userName: string,
    password: string,
    gender: string,
    dateOfBirth: Date,
    img: string,
    place: string,
    loggedin: boolean,
    userNameErr: boolean,
    pwdErr: boolean
}

export default class User extends React.Component<IUserProps, IUserState> {
    constructor (props : string | Date) {
        super(props);
        this.state = {
            userName: "",
            password: "",
            gender: "",
            dateOfBirth: new Date(""),
            img: "",
            place: "",
            loggedin: false,
            userNameErr: false,
            pwdErr: false
        }
    }

    submitForm = (event: any): void => {
        event.preventDefault();

        this.setState({
            pwdErr: false,
            userNameErr: false
        });
            
        const userNameNode: HTMLInputElement | null = document.querySelector("[name='user-name']");
        const passwordNode: HTMLInputElement | null = document.querySelector("[name='password']");

        if ((userNameNode !== null) && (passwordNode !== null)) {
            for (let user in userPwdDB) {
                if (user === userNameNode.value) {
                    if (userPwdDB[user] === passwordNode.value) {
                        this.setState({
                            userName: user,
                            gender: userGenderDB[user],
                            dateOfBirth: userDobDB[user],
                            img: userPhotoDb[user],
                            place: userPlaceDb[user],
                            loggedin: true,
                            pwdErr: false,
                            userNameErr: false
                        });
                        return;
                    } else {
                        this.setState({
                            pwdErr: true
                        });
                        return;
                    }
                }
            }
            this.setState({
                userNameErr: true
            })
        }
    }

    logout = (event: any) => {
        event.preventDefault();

        this.setState({
            userName: '',
            gender: '',
            dateOfBirth: new Date(),
            img: '',
            place: '',
            loggedin: false,
            userNameErr: false,
            pwdErr: false
        })
    }

    LoginForm = () => (
        <div>
            <Navbar />
            <Grid textAlign='center' style={{ height: '100vh' }} verticalAlign='middle'>
                <Grid.Column style={{ maxWidth: 450 }}>
                    <Header as='h2' color='teal' textAlign='center'>
                        <Image src='data:image/png;base64,' /> 
                        Log-in to your account
                    </Header>
                    <Form size='large'>
                        <Segment stacked>
                            <Form.Input 
                            fluid 
                            icon='user'
                            iconPosition='left' 
                            placeholder='UserName' 
                            name='user-name'
                            error={this.state.userNameErr ? "User Name doesn't exist!" : null} 
                            />
                            <Form.Input
                            fluid
                            icon='lock'
                            iconPosition='left'
                            placeholder='Password'
                            type='password'
                            name='password'
                            error={this.state.pwdErr ? "Password incorrect!" : null}
                            />
                            <Button color='teal' fluid size='large' onClick={this.submitForm}>
                            Login
                            </Button>
                        </Segment>
                    </Form>

                </Grid.Column>
            </Grid>
        </div>
      );

    render() {
        if (this.state.loggedin) {
            const user = { profile: this.state }
            return (
                <div>
                    <Navbar />
                    <Grid textAlign='center' style={{ height: '100vh' }} verticalAlign='middle'>
                        <Grid.Column style={{ maxWidth: 290 }}>
                            <Profile {...user} />
                            <Button color='teal' fluid onClick={this.logout}>
                            Logout
                            </Button>
                        </Grid.Column>
                    </Grid>
                </div>
            );
        } else {
            return this.LoginForm();
        }
    }
}